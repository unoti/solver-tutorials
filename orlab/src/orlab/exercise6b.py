...
def tank_solve_b(
    ...
    *,
    ...
    """Assign demands to tanks while minimizing stranded capacity.
    ...
    Args:
        ...
        tank_usage_penalty: Small positive weight that discourages touching
            additional tanks.  The default is ``1.0`` which is tiny compared to
            the stranded capacity units, but helps the solver break ties.
    ...
    use_tank = {
        t: solver.BoolVar(f"use_{t}") for t in tank_indices
    }
    ...
    objective = solver.Sum(stranded.values()) + tank_usage_penalty * solver.Sum(
        use_tank.values()
    )
